class BitBoard {
    #verticalMask = 0x7e7e7e7e7e7e7e7en;
    #horizonMask  = 0x00ffffffffffff00n;
    #allSideMask  = 0x007e7e7e7e7e7e00n;
    #black = 0;
    ...
    makeLegalBoard() {
        ...
        let self = this.turn === this.#black ? this.boardBlack : this.boardWhite;
        let opponent = this.turn === this.#black ? this.boardWhite : this.boardBlack;
        ...
        let result = _lookup(this.#verticalMask, (pos) => pos << 1n);
        result |= _lookup(this.#verticalMask, (pos) => pos >> 1n);
        result |= _lookup(this.#horizonMask, (pos) => pos << 8n);
        result |= _lookup(this.#horizonMask, (pos) => pos >> 8n);
        result |= _lookup(this.#allSideMask, (pos) => pos << 9n);
        result |= _lookup(this.#allSideMask, (pos) => pos << 7n);
        result |= _lookup(this.#allSideMask, (pos) => pos >> 7n);
        result |= _lookup(this.#allSideMask, (pos) => pos >> 9n);
    ...
    // ひっくり返せる石を返す
    flipList(pos, color) {
        let self = color === this.#black ? this.boardBlack : this.boardWhite;
        let opponent = color === this.#black ? this.boardWhite : this.boardBlack;
